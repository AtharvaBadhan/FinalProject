...
// project-imports
import AlertVehicleDelete from './AlertVehicleDelete';
...
// constant
const getInitialValues = (vehicle: FormikValues | null) => {
  const newVehicle = {
    name: '',
    email: '',
    location: '',
  ...
  if (vehicle) {
    newVehicle.name = vehicle.fatherName;
    newVehicle.location = vehicle.address;
    return _.merge({}, newVehicle, vehicle);
  }
...
const AddVehicle = ({ vehicle, onCancel }: Props) => {
  const theme = useTheme();
  const isCreating = !vehicle;

  const [selectedImage, setSelectedImage] = useState<File | undefined>(undefined);
  const [avatar, setAvatar] = useState<string | undefined>(
    avatarImage(`./avatar-${isCreating && !vehicle?.avatar ? 1 : vehicle.avatar}.png`)
  );
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { setSubmitting }) => {
      try {
        ...
        if (vehicle) {
          // dispatch(updateVehicle(vehicle.id, newVehicle)); - update
          dispatch(
            openSnackbar({
              open: true,
          ...
          // dispatch(createVehicle(newVehicle)); - add
          dispatch(
            openSnackbar({
              open: true,
  ...
  return (
    <>
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={8}>
                  <Grid container spacing={3}>
                    <Grid item xs={12}>
                      <Stack spacing={1.25}>
                        <InputLabel htmlFor="vehicle-name">Name</InputLabel>
                        <TextField
                          fullWidth
                          id="vehicle-name"
                    ...
                    <Grid item xs={12}>
                      <Stack spacing={1.25}>
                        <InputLabel htmlFor="vehicle-email">Email</InputLabel>
                        <TextField
                          fullWidth
                          id="vehicle-email"
                          placeholder="Enter Vehicle Email"
                          {...getFieldProps('email')}
                    ...
                    <Grid item xs={12}>
                      <Stack spacing={1.25}>
                        <InputLabel htmlFor="vehicle-orderStatus">Status</InputLabel>
                    ...
                    <Grid item xs={12}>
                      <Stack spacing={1.25}>
                        <InputLabel htmlFor="vehicle-location">Location</InputLabel>
                        <TextField
                          fullWidth
                          id="vehicle-location"
                          multiline
      ...
      {!isCreating && <AlertVehicleDelete title={vehicle.fatherName} open={openAlert} handleClose={handleAlertClose} />}
    </>
  );
};
